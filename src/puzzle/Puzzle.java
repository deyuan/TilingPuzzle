 ...
 *          1) Add spin
 *
 * @version 0.3 11/14/2014
 *          1) Change the data from int to char;
 *
 * @version 0.2 11/14/2014
 *          Description: 1) No spin is allowed;
 *          2) Color is allowed.
 *
 * @see util.Tile
 * @see util.Iterator
...
public class Puzzle {
	...
	/**
	 * An array of record if a position of the board has been occupied by a
	 * tile.
	 */
	...
	private List<List<int[]>> backTracking() {
		...
		/**
		 * The stack is used for backtracking. The index of tiles are store in
		 * this stack, and their positions are stored in curPos.
		 */
		Stack<Integer> stack = new Stack<Integer>();

		/**
		 * Available positions of all tiles from (0, 0) to (length, width) The
		 * Iter is in a format of (y, x).
		 */
		Iterator it[] = new Iterator[number];

		/**
		 * Note that methods of cover, uncover and check are designed for spin,
		 * for no spin scenario, the data is stored at the end of pattern list.
		 * So pattern[] is just for no spin scenario to access data.
		 */
		int pattern[] = new int[number];
		for (int i = 0; i < number; i++) {
			it[i] = new Iterator(width - tileList.get(i).w + 1, length
					- tileList.get(i).l + 1);
		...
		while (top >= 0) {
			...
			/*
			 * If the tile is satisfied, then store the status; or cover the
			 * tile and push it.
			 */
			if (check(top, it[top].getPos(), pattern[top])) {
				/*
				 * If current is the last tile, restore the position and add it
				 * to metaList.
				 */
				if (top == number - 1) {
					...
					/*
					 * If backtrack to the first one and there is no next
					 * position, then top = -1, end.
					 */
			...
			/*
			 * If the tile is not satisfied, then try next available position if
			 * there is, or backtrack to upper layer.
			 */
			else {
				while (top >= 0 && !it[top].hasNext()) {
					it[top--].reset();
	 ...
	 *
	 * @param index
	 *            tile in the tileList
	 * @param pos
	 *            [y, x]
	 */
	private void cover(int index, int pos[], int p) {
		Iterator it = new Iterator(tileList.get(index).spattern.get(p).length,
				tileList.get(index).spattern.get(p)[0].length);
		while (it.isValid()) {
			int ds[] = it.next();
			if (tileList.get(index).spattern.get(p)[ds[0]][ds[1]] != ' ')
				occupied[pos[0] + ds[0]][pos[1] + ds[1]] = tileList.get(index).spattern
						.get(p)[ds[0]][ds[1]];
		}
	}
	...
	/**
	 * Check if a tile in the tileList at the position of pos[y, x] overlaps
	 * with the board.
	 *
	 * @param index
	 *            the index in the tileList
	 * @param pos
	...
	private boolean check(int index, int pos[], int p) {
		...
		while (it.isValid()) {
			int ds[] = it.next();
			/**
			 * If the position has been occupied, or the color is different from
			 * board color, return false.
			 */
			if (tileList.get(index).spattern.get(p)[ds[0]][ds[1]] != ' ') {
				if (occupied[pos[0] + ds[0]][pos[1] + ds[1]] != ' '
						|| board[pos[0] + ds[0]][pos[1] + ds[1]] != tileList
								.get(index).spattern.get(p)[ds[0]][ds[1]])
					return false;
			}
