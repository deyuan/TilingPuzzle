 ...
 *
 */
public class DisplayPuzzle extends JPanel implements ActionListener {
	...
	private final Color color[] = { Color.cyan, Color.blue, Color.green,
			Color.red, Color.yellow, Color.darkGray, Color.lightGray,
			Color.orange, Color.magenta, Color.gray, Color.pink, Color.white };
	 ...
	 */
	@Deprecated
	public DisplayPuzzle(char b[][], List<Tile> t, List<List<int[]>> p,
			boolean s) {
		super(null);
	}

	public DisplayPuzzle(Puzzle p) {
		 ...
		 */
		sizeBlock = Math.min(
				(displaySize[0] - origin[0] - 5) / puzzle.getBoard()[0].length,
				(displaySize[1] - origin[1] - 5) / puzzle.getBoard().length)
				- gridWidth;
		...
		cbEnableSpin.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cbEnableSpin.isSelected())
					puzzle.setEnableSpin(true);
				else
					puzzle.setEnableSpin(false);
			}
	...
	private void displayTilesSpin(int id) {
		...
		for (int i = 0; i < number; i++) {
			...
			for (int j = 0; j < puzzle.getTileList().get(i).spattern.get(pos
					.get(i)[2]).length; j++) {
				for (int k = 0; k < puzzle.getTileList().get(i).spattern
						.get(pos.get(i)[2])[0].length; k++) {
					if (puzzle.getTileList().get(i).spattern.get(pos.get(i)[2])[j][k] != ' ') {
						JPanel block = new JPanel();
						block.setBackground(color[i]);
						block.setSize(sizeTile, sizeTile);
						int x = originTile[0] + (pos.get(i)[1] + k) * sizeTile;
